...
LOOP:
    ...
    rrncf dato_C,f           ; C = C >> 1 
    ; Realizar la operacion (A)'
    comf  dato_A,f           ; A = (A)'
    ; Realizar la operacion (B)'
    comf dato_B,f            ; B = (B)'
    ; Realizar la operacion B'C
    movf  dato_B,W           ; W = B'
    andwf dato_C,f           ; C = B' & C
    ; Realizar la operacion A' + (B'C)
    movf  dato_C,W           ; W = (B'C)
    iorwf dato_A,F           ; A = A' + B'C
    ; Mostrar resultado en salida
    btfss dato_A,0           ; ¿Dato_B == 1?
